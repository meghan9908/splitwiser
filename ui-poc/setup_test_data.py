...
class SplitWiserTestSetup:
    ...
    def signup_user(self, name: str, email: str, password: str) -> Dict[str, Any]:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Failed to create user {name}: Status {response.status_code}, {response.text}")
    ...
    def join_group(self, user_token: str, join_code: str, user_name: str) -> bool:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Failed to join group {join_code} for {user_name}: {response.text}")
            return False
    
    def get_existing_expenses(self, user_token: str, group_id: str) -> List[Dict[str, Any]]:
    ...
    def create_expense(self, user_token: str, group_id: str, description: str, 
                      amount: float, splits: List[Dict], split_type: str = "equal") -> Dict[str, Any]:
    ...
    def create_expense_if_not_exists(self, user_token: str, group_id: str, description: str, 
                                   amount: float, splits: List[Dict], split_type: str = "equal") -> Dict[str, Any]:
        ...
        return self.create_expense(user_token, group_id, description, amount, splits, split_type)
    ...
    def setup_groups(self):
        ...
        existing_groups = self.get_existing_groups(self.users["Alice Johnson"]["access_token"])
        ...
        if house_group:
            ...
            if house_group:
                ...
                self.join_group(self.users["Bob Smith"]["access_token"], house_group["joinCode"], "Bob Smith")
                self.join_group(self.users["Charlie Brown"]["access_token"], house_group["joinCode"], "Charlie Brown")
                time.sleep(1)
        
        # Group 2: Trip to Goa (Alice, Diana, Eve)
        existing_groups_diana = self.get_existing_groups(self.users["Diana Prince"]["access_token"])
        ...
        if trip_group:
            ...
            if trip_group:
                ...
                self.join_group(self.users["Alice Johnson"]["access_token"], trip_group["joinCode"], "Alice Johnson")
                self.join_group(self.users["Eve Wilson"]["access_token"], trip_group["joinCode"], "Eve Wilson")
                time.sleep(1)
        
        # Group 3: Office Lunch Group (Bob, Charlie, Diana, Eve)
        existing_groups_bob = self.get_existing_groups(self.users["Bob Smith"]["access_token"])
        ...
        if lunch_group:
            ...
            if lunch_group:
                ...
                self.join_group(self.users["Charlie Brown"]["access_token"], lunch_group["joinCode"], "Charlie Brown")
                self.join_group(self.users["Diana Prince"]["access_token"], lunch_group["joinCode"], "Diana Prince")
                self.join_group(self.users["Eve Wilson"]["access_token"], lunch_group["joinCode"], "Eve Wilson")
                time.sleep(1)
